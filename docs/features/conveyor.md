...
컨베이어의 물리 이송과 checkpoint·검사 이미지 발행은 GPU PC 1의 Isaac Sim이
소유한다. 개인 PC 1의 영상 기반 수확 인식과 GPU PC 1의 Lula RRT 경로 계획은
컨베이어 품질 검사와 분리되며, RViz는 컨베이어와 수확 상태를 원격 표시하는
용도로만 사용한다.

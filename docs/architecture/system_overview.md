# 시스템 개요

## 목적

Isaac Sim에서 단일 사과나무의 사과를 협동로봇으로 수확하고 컨베이어에 배치한 뒤, 영상 기반 품질 등급을 산출한다. 수확 위치 인식과 품질 검사는 기능적으로 분리한다.

## 전체 흐름

```text
사과 pose 획득
  → 충돌 없는 접근 경로 계획
  → 파지
  → 손목 45° 회전
  → 로봇의 직선 당김
  → 줄기 분리
  → 컨베이어 1 저상 배치
  → 컨베이어 2 촬영·추론
  → 컨베이어 3 가상/실제 푸셔 분류
```

## 주요 구성

- 로봇: Doosan M0617, 6DOF
- 그리퍼: AGS-001-MTCP 3-finger soft gripper의 rigid finger + compliant contact 근사
- 카메라: 베이스, arm, 컨베이어 상단에 Intel RealSense D455
- 환경: 사과나무 1그루, 사과 1개, 로봇 1개, 1차 MVP용 컨베이어 3모듈
- 모션: LulaKinematicsSolver 기반
- 통신: ROS 2 Jazzy, Fast DDS, `/clock` 기반 simulation time

## PC 역할

- GPU PC 1: Isaac Sim, 물리, 센서, tracker, ROS 2 Bridge
- GPU PC 2: 품질 영상 추론 및 사과 단위 결과 통합
- 개인 PC 1: 로봇 계획·상태 머신 및 개발 지원
- 개인 PC 2: 분류 제어, 모니터링 및 푸셔 선택

역할은 통합 시험 결과에 따라 조정할 수 있다.

## 기능 경계

- 수확용 인식은 사과 위치와 접근 정보를 수확 기능에 전달한다.
- 수확 기능은 품질을 판정하지 않는다.
- 품질 분류는 컨베이어 상단 카메라에서 독립적으로 수행한다.
- 푸셔 물리 구현은 2차 개발이며 MVP에서는 가상 위치 매칭만 검증한다.

## 성공 상태

수확, 컨베이어 배치, 품질 판정 및 가상 분류 위치 확인이 모두 완료되어야 한 번의 실행을 성공으로 본다.

